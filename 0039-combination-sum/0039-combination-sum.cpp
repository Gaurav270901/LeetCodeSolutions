...
public: 
    ...
    void backtrack(vector<int>& candidates , vector<int> &current , int currentSum , int target , int index ){
       //
...
//first we will include current element in our addition and then take that element again in repeat call as seen in backtrack we are sentting the same index
        ...
        backtrack(candidates,current,currentSum + candidates[index] , target,index );

//now we will exclude that element from combination and take next index
        current.pop_back();
        backtrack(candidates , current , currentSum  , target , index+1);
    }
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        vector<int> current ;

        backtrack(candidates , current , 0 , target ,0 );
        return ans ;
    }
};